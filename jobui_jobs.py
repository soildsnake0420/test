...
class JobuiSpider(scrapy.Spider):
    ...
    def parse_job(self, response):
        ...
        # 用fin方法提取出公司名称
        datas = bs.find_all('div', class_="c-small-job-list")
        ...
        for data in datas:
            ...
            items['position'] = data.find("a", class_="job-name").text

            # 提取出工作地点，并把这个数据放回JobuiItem类的address属性里
            items['address'] = data.find('span', class_="col80").text

            # 提取出招聘要求，并把这个数据放回JobuiItem类的detail属性里
            items['detail'] = data.find('span', class_="job-small-desc").text
            '''
            # 用yield语句把item传递给引擎
            yield items
